...
contains
  ...
  ! subroutine calculate(this, totGridDat, pCopyBuffers, speciesGridsDat, coeffs, levelIndex,&
  !     & orbitalToAtom, orbitalToSpecies, nRegions, totGridsDat, addDensities)

  !   !> MolecularOrbital instance
  !   type(TMolecularOrbital), intent(in) :: this

  !   !> Total grid instance
  !   type(TGridData), intent(in) :: totGridDat

  !   !> Pointer to target array that holds the volumetric data of copied total grids
  !   real(dp), intent(in), pointer :: pCopyBuffers(:,:,:,:)

  !   ! !> Pointer to target array that holds the volumetric data of copied total grids
  !   ! real(dp), intent(inout), target :: copyBuffers(:,:,:,:)

  !   !> Orbital-Species grid data instances
  !   type(TGridData), intent(in) :: speciesGridsDat(:)

  !   !> Summation coefficients, i.e. eigenvectors or free occupations
  !   real(dp), intent(in) :: coeffs(:,:)

  !   !> Index Mapping from state index to [Level, KPoint, Spin]
  !   integer, intent(in) :: levelIndex(:,:)

  !   !> Index Mapping from orbitals to atoms
  !   integer, intent(in) :: orbitalToAtom(:)

  !   !> Index Mapping from orbitals to species
  !   integer, intent(in) :: orbitalToSpecies(:)

  !   !> Number of parallel regions used by omp
  !   integer, intent(in) :: nRegions

  !   !> Grid caches, holding state-specific total grid instances
  !   type(TGridData), intent(out), allocatable :: totGridsDat(:)

  !   !> Add densities instead of wave functions
  !   logical, intent(in), optional :: addDensities

  !   call subgridsToGlobalGrid(totGridDat, speciesGridsDat, this%coords, coeffs, levelIndex,&
  !       & orbitalToAtom, orbitalToSpecies, nRegions, pCopyBuffers, totGridsDat,&
  !       & addDensities=addDensities)
